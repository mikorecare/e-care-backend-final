...
// Function to check for bad words in a text
const containsBadWords = (text) => {
  // Escape special characters for regex
  const escapedBadWords = badWords.map(word => word.replace(/[.*+?^=!:${}()|\[\]\/\\]/g, "\\$&"));
  
  const regex = new RegExp(`\\b(${escapedBadWords.join('|')})\\b`, 'i'); // Create a regex pattern to match bad words
...
router.put('/:id', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // Check if the comment contains any bad words before updating
    if (comments && containsBadWords(comments)) {
      return res.status(400).json({
        error: 'Your comment contains inappropriate language. Please remove it and try again.',
      });
    }
